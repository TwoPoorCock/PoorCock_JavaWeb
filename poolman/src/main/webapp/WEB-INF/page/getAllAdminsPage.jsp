    ...
    $(document).ready(function () {
            ...
            "columnDefs": [
                {"name": "id", "targets": 0},
                {
                    ...
                    "render": function (data, type, row) {
                        return '<a href="' + '<%=basePath%>user/toEditAdminPage.htm?id=' + row.id + '">' + row.name + '</a>';
                    }
                },
                {"name": "userName", "targets": 2},
                {"name": "passWord", "targets": 3},
        ...
        function enableAndDisableStatus(title, status, id) {
            $.confirm({
                ...
                confirm: function () {
                    $.ajax({
                        url: "<%=basePath%>user/batchEnableOrDisableSystemUser.htm",
                        type: "post",
                        data: {
                            systemUserIds: id,
                            status: status
