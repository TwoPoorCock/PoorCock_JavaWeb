...
public class AppPersonInfoController extends AppController {
	...
	private static Logger logger = Logger.getLogger(AppPersonInfoController.class);
	
	@Resource
	private IAppPersonInfoService appPersonInfoService;
	
	@RequestMapping(value = "/editPersonalInfo", method = { RequestMethod.POST, RequestMethod.GET })
	public void editPersonalInfo(){
		...
		try {
			...
			logger.info("userId="+userId+" gender="+gender+" phone="+phone
					+" height="+height+" weight="+weight+" weiteng="+weiteng+" kouqiangky="+kouqiangky
					+" yayingcx="+yayingcx+" jianfei="+jianfei);
			...
			if(TextHelper.isNullOrEmpty(userId)){
				...
				if(temp>0){
					...
					logger.info("userId="+user2.getId()+" gender="+user2.getGender()+" phone="+user2.getPhone()+" height="+user2.getHeight()
					+" weight="+user2.getWeight()+" kqky="+user2.getKouqiangky()+" yycx="+user2.getYayingcx()+" wt="+user2.getWeiteng()+" jf="+user2.getJianfei());
				}else{
					data.setStatus(FAIL);
