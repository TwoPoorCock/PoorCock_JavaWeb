...
public class BlueMobiConstant {
	 ...
	 * 获取项目在磁盘上面的物理路径
	 */
	public static final String WEB_SITE_ROOT_PATH = BlueMobiConstant.class
			.getResource("/")
			.getPath()
			.replaceAll("%20", " ")
			.substring(
					0,
					BlueMobiConstant.class.getResource("/").getPath()
							.replaceAll("%20", " ").indexOf("WEB-INF"));

	/**
	 * 获取项目在磁盘上面的物理路径
	 */
	public static final String WEB_CLASSPATH = BlueMobiConstant.class
			.getResource("/").getPath().replaceAll("%20", " ");

	/**
	 * 图片文件格式限制
	 */
	public static final String[] IMAGE_FORMAT_ARRAY = new String[] { ".gif",
			".png", ".jpg", ".jpeg", ".bmp" };// 全站中引用的图片格式
