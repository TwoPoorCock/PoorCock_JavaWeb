...
public class FileHelper {
	...
	public static String getTimeFileName() {
		...
		return df.format(date)
				+ (new Random().nextFloat() + "").substring(2, 6);
	}
	...
	public static String getTimeFileName(String fileType) {
		...
		DateFormat df = new SimpleDateFormat("yyyyMMddHHmmss");
		return df.format(date)
				+ (new Random().nextFloat() + "").substring(2, 6) + fileType;
	}
	 ...
	 * @throws IOException
	 */
	public static boolean updateFileName(File oldFile, File newFile)
			throws IOException {
	 ...
	 * @return
	 */
	public static boolean downloadWebFile(String webFileUrl, String fileName,
			String parentPath) {
		InputStream in = null;
		...
		try {
			...
			while (ma.find()) {
				t = ma.group();
				webFileUrl = webFileUrl.replace(t,
